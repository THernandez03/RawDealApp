...
function chatHeartbeat(){
	...
	$j.ajax({
	  url: "/rawdealApp/gameplays/chatheartbeat",
	  cache: false,
	  dataType: "json",
...
function closeChatBox(chatboxtitle) {
	...
	$j.post("/rawdealApp/gameplays/closechat", { chatbox: chatboxtitle} , function(data){	
	});
...
function checkChatBoxInputKey(event,chatboxtextarea,chatboxtitle) {
	...
	if(event.keyCode == 13 && event.shiftKey == 0)  {
		...
		if (message != '') {
			$j.post("/rawdealApp/gameplays/sendchat", {to: chatboxtitle, message: message} , function(data){
...
function startChatSession(){  
	$j.ajax({
	  url: "/rawdealApp/gameplays/startchatsession",
	  cache: false,
	  dataType: "json",
	  success: function(data) {
